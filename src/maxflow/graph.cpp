...
Graph::Graph(long nb_nodes, long nb_arcs, void (*err_function)(char *))
{
	error_function = err_function;
	node_block = new Block<node>(nb_nodes, error_function);
	arc_block  = new Block<arc>(nb_arcs, error_function);
	flow = 0;
}
...
void Graph::reset_and_keep_graph_structure()
{
    TIME=0;
    // first, reset the list of outcoming arcs for each node
    for (node *i=node_block->ScanFirst(); i; i=node_block->ScanNext())
    {
	i -> first = NULL;
	i -> tr_cap = 0;
	i -> TS = 0;
