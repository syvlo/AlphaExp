...
class Graph
{
	...
	/* Constructor. The first two arguments can be used to
	   provide an estimation of the number of nodes/arcs to
	   be used. Optional argument is the pointer to the
	   function which will be called if an error occurs;
	   an error message is passed to this function. If this
	   argument is omitted, exit(1) will be called. */
	Graph(long nb_nodes, long nb_arcs, void (*err_function)(char *) = NULL);
	...
	/* Reset the graph but keep its structure, i.e. keep the nodes, reset their weights
	   and free the arcs */
	void reset_and_keep_graph_structure();
	
	/* Reset the graph (all nodes and arcs are lost, but further node/arc insertion will
	 * not require reallocating the space already used) */
	void reset();
	...
	typedef struct node_st
	{
	    ...
	    captype			tr_cap;		/* if tr_cap > 0 then tr_cap is residual capacity of
							   the arc SOURCE->node
							   otherwise         -tr_cap is residual capacity of
							   the arc node->SINK
							 */
	} node;
